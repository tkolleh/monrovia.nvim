...
function M.from(spec)
  local ovr = require("monrovia.override").groups
  local config = require("monrovia.config").options

  if not spec then
    print("there is no spec")
  end
  local editor = require("monrovia.group.editor").get(spec, config)
  local syntax = require("monrovia.group.syntax").get(spec, config)

  local result = collect.deep_extend(editor, syntax)

  local default_enable_value = config.module_default
  local mod_names = require("monrovia.config").module_names
...
function M.load(name)
  name = name or require("monrovia.config").fox
  return M.from(require("monrovia.spec").load(name))
end
